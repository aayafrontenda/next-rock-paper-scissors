...
const ROOMS_MAX_COUNT = 5;
const MOVE_TIME = 1000;
const users: Record<string, UserInfo> = {};
const rooms: Room[] = [];
for (let i = 0; i < ROOMS_MAX_COUNT; i++) {
  rooms.push({
...
io.on("connection", (socket: Socket<ServerEventsMap, ServerEventsMap>) => {
  ...
  socket.on("joinRoom", (data) => {
    ...
    const randomRoom = Math.floor(Math.random() * freeRooms.length);
    ...
    if (randomRoom !== undefined) {
      ...
      if (rooms[randomRoom].booked === PLAYERS_MAX_COUNT) {
        ...
        io.in(randomRoom.toString()).emit("startTimer", MOVE_TIME);
      }
    }
  });
